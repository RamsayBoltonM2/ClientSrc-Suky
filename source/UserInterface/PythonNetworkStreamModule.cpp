...
PyObject* netSendQuestCommandPacket(PyObject* poSelf, PyObject* poArgs)
{	
	DWORD qidx;
	if (!PyTuple_GetDWORD(poArgs, 0, &qidx))
		return Py_BuildException();

	char * szCmd;
	if (!PyTuple_GetString(poArgs, 1, &szCmd))
		return Py_BuildException();

	CPythonNetworkStream& rns = CPythonNetworkStream::Instance();
	rns.SendQuestCommandPacket(szCmd, qidx);
...
void initnet()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "SendQuestCommandPacket",					netSendQuestCommandPacket,					METH_VARARGS },
