...
void CPythonShop::AddPrivateShopItemStock(TItemPos ItemPos, BYTE dwDisplayPos, long long dwPrice)
{
	...
	TPrivateShopItemTable SellingItem;
	SellingItem.vnum = 0;
	SellingItem.count = 0;
...
long long CPythonShop::GetPrivateShopItemPrice(TItemPos ItemPos)
{
	...
	TPrivateShopItemTable & rShopItemTable = itor->second;
	return rShopItemTable.price;
}
struct ItemStockSortFunc
{
	bool operator() (TPrivateShopItemTable & rkLeft, TPrivateShopItemTable & rkRight)
	{
...
{
	std::vector<TPrivateShopItemTable> ItemStock;
