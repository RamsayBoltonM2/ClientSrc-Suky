...
class CPythonNetworkStream : public CNetworkStream, public CSingleton<CPythonNetworkStream>
{
	...
	public:
		...
		bool SendShopSellPacketNew(UINT bySlot, short byCount);
		...
		bool SendQuestCommandPacket(const char * c_szCmd, DWORD quest_index);
		...
		// Private Shop
		bool SendBuildPrivateShopPacket(const char * c_szName, const std::vector<TPrivateShopItemTable> & c_rSellingItemStock);
