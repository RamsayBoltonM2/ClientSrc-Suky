...
bool CPythonNetworkStream::RecvChatPacket()
{
	...
	else
	{
		...
		else if (CHAT_TYPE_SHOUT == kChat.type || CHAT_TYPE_TEAM == kChat.type)
		{
			char * p = strchr(buf, ':');
		...
		case SHOP_SUBHEADER_GC_START:
			{
				...
				PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "StartShop", Py_BuildValue("(ii)", dwVID, pShopStartPacket->grid_type));
			}
			break;
...
bool CPythonNetworkStream::SendQuestCommandPacket(const char * c_szCmd, DWORD quest_index)
{
	TPacketCGQuestRcv Packet;
	Packet.bHeader = HEADER_CG_QUEST_RECEIVE;
	Packet.questID = quest_index;
	strncpy_s(Packet.msg, c_szCmd, QUEST_INPUT_STRING_MAX_NUM);

	if (!Send(sizeof(Packet), &Packet))
	{
		Tracef("SendQuestCommandPacket Error");
		return false;
	}
...
/////////////////////////////////////////////////////////////////////////
// MyShop
bool CPythonNetworkStream::SendBuildPrivateShopPacket(const char * c_szName, const std::vector<TPrivateShopItemTable> & c_rSellingItemStock)
{
	...
	for (std::vector<TPrivateShopItemTable>::const_iterator itor = c_rSellingItemStock.begin(); itor < c_rSellingItemStock.end(); ++itor)
	{
		const TPrivateShopItemTable & c_rItem = *itor;
